        ...
        if not os.path.exists(name) or not ext == ".m":
            ...
            if mexSuccess:
                ...
                if os.path.exists(name):
                    ...
                    print(
                        "Skipping because cannot find help in mex or mfile for: "
                        + os.path.join(head, basename)
                    )
                    continue
    ...
    with open(filename) as toParse:
        for line in toParse:
            ...
            if sectionMatch:
                ...
                extractedText[sectionName][keyName] = (
                    extractedText[sectionName][keyName] + line
                )
...
def mexhelpextract(outputDir, mexnames):
    ...
    for mexname in mexnames:
        ...
        try:
            # Try Octave first:
            cmd = (
                'octave --no-history --no-window-system --silent --no-gui --eval "%s" > /dev/null'
                % matlabcmd
            )
        ...
        for subFuncName, subFuncText in subfunctions.items():
            ...
            breadcrumb = (
                "##### "
                + formatLinkText("Psychtoolbox", "Psychtoolbox")
                + ">"
            ...
            if seealso:
                ...
                if outputFormat == "markdown":
                    seealso = re.sub(
                        UpperCamelMatch, r"[\g<0>](" + mexname + r"-\g<0>)", seealso
                    )
                elif outputFormat == "mediawiki":
                    seealso = re.sub(
                        UpperCamelMatch, r"[[" + mexname + r"-\g<0>|\g<0>]]", seealso
                    )

                docstring = docstring + "###See also:\n" + seealso

            text = headline + breadcrumb + docstring
            ...
            outputFilename = os.path.join(
                outputDir, mexname + "-" + subFuncName + suffix
            )
            if subFuncName == "__main__":
...
def recursivewalk(outputDir, rootfolder):
    ...
    for root, dirs, files in os.walk(rootfolder):
        if re.search(os.sep + r"\.svn|" + os.sep + "private", root):
            continue
        ...
        mfiles = [
            os.path.join(root, f) for f in files if f.endswith((".m", ".M", _mexext))
        ]
